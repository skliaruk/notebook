...
void main() {
  ...
  void setUpMockDioClientSuccess200() {
    when(mockDioClient.get<Map<String, dynamic>>(any)).thenAnswer((_) async =>
        dio.Response(
            data: json.decode(fixture('note.json')) as Map<String, dynamic>,
            statusCode: 200));
  }
  ...
  void setUpMockDioClientFailure404() {
    when(mockDioClient.get<Map<String, dynamic>>(any)).thenAnswer((_) async =>
        dio.Response(
            data: {'error': 'Error'} as Map<String, dynamic>, statusCode: 404));
  }
  ...
  group('getNote', () {
    ...
    test('should perform a GET request on a URL with number being the endpoint',
      ...
      verify(mockDioClient.get<Map<String, dynamic>>(
          'http://jsonplaceholder.typicode.com/posts/$tNoteId'));
    });
